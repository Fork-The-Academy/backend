...
module.exports = {
    lifecycles: {
        ...
        beforeCreate() {
            contract.methods.returnAllProposals().call().then(n => {
                PROPOSALSCOUNT = n.length
            });
        },
        async afterCreate(data) {
            const response = await createProposalInBlockhain(data.id)
            if(response) {
                await strapi.query('proposals').update({ id: data.id}, {blockchainId: PROPOSALSCOUNT});
                PROPOSALSCOUNT++
                console.log(data)
            }
        },
